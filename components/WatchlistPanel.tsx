...
interface WatchlistPanelProps {
  ...
  watchedCompanyKeys: string[];
  onRemove: (companyKey: string) => void;
}

export default function WatchlistPanel({ events, watchedCompanyKeys, onRemove }: WatchlistPanelProps) {
  const entries = watchedCompanyKeys
    .map((companyKey) => {
      const company = events.find((event) => event.companyId === companyKey || event.company.id === companyKey || event.company.ticker === companyKey)?.company;
      return company ? { company, key: companyKey } : undefined;
    })
    .filter((entry): entry is { company: Company; key: string } => Boolean(entry))
    .filter((entry, index, list) => list.findIndex((item) => item.company.id === entry.company.id || item.company.ticker === entry.company.ticker) === index);
  ...
  return (
    <aside className="rounded-lg border border-line bg-white p-4 shadow-panel">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-surface px-2 py-1 text-xs font-semibold text-muted">{entries.length}개</span>
      </div>
